...
void generate_sample_orders(vector<Order>& orders, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        Order order;
        order.timestamp = base + (random_minutes * 60);
        order.order_id = "ORD" + to_string(i + 1);
...
int main() {
    ...
    cout << "Done! Sorted " << NUM_ORDERS
         << " orders in " << time_taken.count() << " seconds.\n";

    cout << "\n First " << NUM_ORDERS << " Sorted Orders:\n";
    print_first_n_orders(orders, NUM_ORDERS);
